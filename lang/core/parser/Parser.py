...
class Parser:
	def __init__(self, script_text: str, statements: list[Statement]):
	...
	def run(self):
		...
		for statement in self.statements:
			node = parse_instruction_node(self.script_text, statement)
			if isinstance(node, LxSyntaxError):
				...
				if current_node.keyword.data != node.block_name.data:
					e.errors.append(LxSyntaxError("[Parser Error] block ending instruction does not match block opening instruction:\n"
												  + node.block_name.pos.message_pointer(self.script_text) + '\n'
												  + current_node.keyword.pos.message_pointer(self.script_text)))
