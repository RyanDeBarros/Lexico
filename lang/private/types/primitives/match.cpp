...
namespace lx
{
	...
	void Match::assign(const EvalContext& env, Variable o)
	{
		*this = std::move(o).consume_as<Match>(env);
	}

	bool Match::equals(const EvalContext& env, Variable o) const
	{
		return equals(std::move(o).cast(env, data_type()).ref().get<Match>());
	}
