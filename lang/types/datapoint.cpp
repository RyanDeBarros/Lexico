...
namespace lx
{
	...
	{
		return std::visit([&env, &type](auto&& v) { return DataPoint(std::move(v).cast_move(env, type)); }, std::move(_storage));
	}
	...
	void DataPoint::assign(const EvalContext& env, DataPoint&& other)
	{
		DataPoint casted = std::visit([this, &env](auto& o) {
			return std::move(o).cast_move(env, data_type());
		}, other._storage);
		std::visit([&env, &casted](auto& v) { v.assign(env, std::move(std::get<std::decay_t<decltype(v)>>(casted._storage))); }, _storage);
	}
	...
	{
		if (other.can_cast_implicit(data_type()))
			return std::visit([&env, o = std::move(other).cast_move(env, data_type())](const auto& v) { return v.equals(env, o.get<std::decay_t<decltype(v)>>()); }, _storage);
		else
			return false;
	}
